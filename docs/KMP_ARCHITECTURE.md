...
- **Charts & maps stay platform-native** — share the *data* (entries, target zones, heatmap
  ...
  in the UI.
- Navigation: **shared** Compose navigation (Navigation Compose Multiplatform) owns the back
  stack on both platforms. The SwiftUI shell keeps only the tab bar and presents the
  Compose-owned stacks — see §6.1 and §6.2. ~~On iOS navigation is native.~~ *(superseded
  2026-08-03 — see §6.2.)*
...
### 6.2 Navigation Ownership — AMENDED 2026-08-03

**§6.1's "iOS nav is native" clause is superseded. Navigation is now shared: Navigation
Compose Multiplatform (`org.jetbrains.androidx.navigation:navigation-compose`) owns the back
stack on both platforms.** Everything else in §6.1 stands — the Swift shell still holds zero
domain data, screens are still shared Compose, SKIE is still only-when-needed.

Written before the designs existed; the designs changed the calculus. Four reasons:

1. **The design system is Material by intent, not by accident.** The Figma app bar
   (`160:9069`) is an M3 *small, flat* top app bar — `titleLarge` Roboto 22/28, `onSurface`
   `#1C1B1B`, left-aligned title, Material `arrow_back`. A SwiftUI `NavigationStack` centres
   the title and offers a "‹ Back" affordance. Native chrome would fight the design, and a
   Liquid Glass bar over M3 cards reads as foreign *within a single screen*, which is exactly
   where users notice inconsistency.
2. **CMP already covers the behaviour that actually matters.** iOS back-swipe is translated
   into `popBackStack` with a native-like transition, on by default
   (`ComposeUIViewController(configure = { enableBackGesture = false })` to disable). ⚠️
   Setting a custom `enterTransition`/`exitTransition` on `NavHost` suppresses it.
3. **One composition beats N.** Native nav means one `ComposeUIViewController` per pushed
   level: a fresh Recomposer, a fresh `CompositionLocal` chain (so `LocalStrings` +
   `AircastingTheme` must be re-provided per route or `LocalStrings.current` silently falls
   back to `EnStrings`), and a fresh `ViewModelStore` per level. That last one bites: nothing
   is shared between levels, so the 3-step connection wizard (`179:11288` → `169:1571` →
   `180:11156`) needs a hand-managed Koin scope instead of a nav-graph-scoped ViewModel, and
   popping discards state that Android's `NavHost` would restore.
4. **Liquid Glass auto-adoption does not reach Compose.** Recompiling against the iOS 26 SDK
   auto-applies Liquid Glass to *system controls* only; Skia-drawn Compose gets none, and
   `.glassEffect()` can't reach inside a Compose host. So native chrome is the only way to
   get it — and per §6.1 we don't want it yet anyway.

**Structure.** Stacks that cover the shell (no bottom bar in the design) are sibling
destinations of the shell, not children of a tab:

```
ANDROID                       iOS
NavHost(root)                 SwiftUI TabView          ← tab bar only
 ├─ ShellRoute                 ├─ ComposeTab(.home) ──onOpenSettings──┐
 │   └─ tabs + AircastingNavBar│                                      │
 └─ SettingsRoute.*            └─ .fullScreenCover ───────────────────┘
                                   └─ SettingsViewController(onClose)
                                       └─ same shared settings NavHost
```

The bottom bar lives *inside* the shell destination, so it disappears on Settings for free.
On iOS the cover sits above `TabView`, which is what hides the tab bar — note a native
`NavigationStack` push would *not* have hidden it either without
`.toolbar(.hidden, for: .tabBar)`.

Swift learns exactly two things: "open settings" and "settings dismissed". No per-route
bridging, so `SettingsRoute` never has to be `Hashable` in Swift.

**Known inconsistency, not yet resolved.** iOS still uses SwiftUI `TabView` with SF Symbols
(`house`, `map`, `plus.circle`, …) rather than the designed `AircastingNavBar` and its own
icons (`ic_home`, `ic_map`, …). So iOS ships different nav icons than Figma specifies, and on
an iOS 26 SDK build gets a glass tab bar under Material content. The coherent endgame is
moving iOS to the shared `AircastingNavBar` too — deliberately deferred, decide separately.

**Revisit trigger:** if native nav *feel* (UIKit pop physics, large-title collapse, Liquid
Glass) becomes a product requirement, or if a screen's CMP perf is unacceptable. The §6.1
escape hatch is unchanged — the shell already hosts native, so one screen can go native
without repricing the app.
...
1. ~~**UI strategy.**~~ **RESOLVED — see §6.1 and §6.2:** native SwiftUI shell + shared Compose
   screens + SKIE-when-needed; **nav shared via Navigation Compose Multiplatform on both
   platforms** (§6.2 amends §6.1's native-iOS-nav clause); Liquid Glass deferred.
